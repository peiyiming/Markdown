...
class MarkdownEditorActivity : AppCompatActivity() {
    ...
    private fun showPreview(saveBeforeRender: Boolean = true) {
        ...
        if (saveBeforeRender) saveDocument()
    ...
    private fun setModeSelection(mode: EditorMode) {
        ...
        editButton.alpha = if (mode == EditorMode.EDIT) 1f else .55f
        previewButton.alpha = if (mode == EditorMode.PREVIEW) 1f else .55f
        liveButton.alpha = if (mode == EditorMode.LIVE) 1f else .55f
    }
    ...
    private fun hasBlockPrefix(line: String, type: BlockType): Boolean = when (type) {
        BlockType.QUOTE -> Regex("^\\s*(?:>\\s?)+").containsMatchIn(line)
        BlockType.UNORDERED -> Regex("^(\\s*)[-+*]\\s+").containsMatchIn(line)
        BlockType.ORDERED -> Regex("^(\\s*)\\d+[.)]\\s+").containsMatchIn(line)
        else -> false
    }

    private fun removeBlockPrefix(line: String, type: BlockType): String = when (type) {
        BlockType.QUOTE -> line.replaceFirst(Regex("^(\\s*)>\\s?"), "$1")
        BlockType.UNORDERED -> line.replaceFirst(Regex("^(\\s*)[-+*]\\s+"), "$1")
        BlockType.ORDERED -> line.replaceFirst(Regex("^(\\s*)\\d+[.)]\\s+"), "$1")
        else -> line
    }

    private fun addBlockPrefix(line: String, type: BlockType, orderedIndex: Int): String {
        val indent = line.takeWhile { it == ' ' || it == '\t' }
        val body = line.substring(indent.length)
        return when (type) {
            BlockType.QUOTE -> indent + "> " + body
            BlockType.UNORDERED -> indent + "- " + body
            BlockType.ORDERED -> indent + "$orderedIndex. " + body
            else -> line
        }
    }
    ...
    private fun toggleWrap(prefix: String, suffix: String) {
        ...
        if (selected.isNotEmpty() && selected.startsWith(prefix) && selected.endsWith(suffix) && selected.length >= prefix.length + suffix.length) {
        ...
        if (isSelectionWrapped(content, start, end, prefix, suffix)) {
            suppressEditorWatcher = true
            content.delete(end, end + suffix.length)
            content.delete(start - prefix.length, start)
        ...
        if (selected.isEmpty()) {
            suppressEditorWatcher = true
            content.insert(start, prefix + suffix)
            editor.setSelection(start + prefix.length)
            suppressEditorWatcher = false
            notifyEditorMutation()
        } else {
            replaceSelection(start, end, prefix + selected + suffix, start + prefix.length, start + prefix.length + selected.length)
        }
    }

    private fun isSelectionWrapped(content: Editable, start: Int, end: Int, prefix: String, suffix: String): Boolean {
        if (start < prefix.length || end + suffix.length > content.length) return false
        if (content.substring(start - prefix.length, start) != prefix || content.substring(end, end + suffix.length) != suffix) return false
        if (prefix == "*" && suffix == "*") {
            val before = start - 2
            val after = end + 1
    ...
    private fun insertLink() {
        ...
        if (selected.isEmpty()) {
            ...
            val url = "https://"
            val value = "[$label]($url)"
            replaceSelection(start, end, value, start + 1, start + 1 + label.length)
        } else {
            val url = "https://"
            val value = "[$selected]($url)"
            val urlStart = start + selected.length + 3
            replaceSelection(start, end, value, urlStart, urlStart + url.length)
        }
    }
    ...
    private fun continueMarkdownBlock(content: Editable, newlineIndex: Int) {
        if (isInsideCodeFence(newlineIndex)) {
            continueCodeFence(content, newlineIndex)
            return
        }
        ...
        if (ordered != null) {
            continueListBlock(content, previousLineStart, newlineIndex, ordered.groupValues[4], ordered.groupValues[1], "${ordered.groupValues[2].toIntOrNull()?.plus(1) ?: 2}${ordered.groupValues[3]} ")
            return
        }
        val unordered = Regex("^(\\s*)([-+*])\\s*(.*)$").matchEntire(previous)
        if (unordered != null) {
            continueListBlock(content, previousLineStart, newlineIndex, unordered.groupValues[3], unordered.groupValues[1], unordered.groupValues[2] + " ")
            return
        }
    ...
    private fun continueListBlock(content: Editable, lineStart: Int, newlineIndex: Int, body: String, indent: String, nextMarker: String) {
        ...
        if (body.trim().isEmpty()) {
            if (indent.isNotEmpty()) {
                content.replace(lineStart, newlineIndex + 1, indent)
                editor.setSelection(lineStart + indent.length)
            } else {
                content.delete(lineStart, newlineIndex + 1)
                editor.setSelection(lineStart)
            }
        } else {
            val prefix = indent + nextMarker
            content.insert(newlineIndex + 1, prefix)
            editor.setSelection(newlineIndex + 1 + prefix.length)
        }
        suppressEditorWatcher = false
        notifyEditorMutation()
    }
    ...
    private fun continueQuoteBlock(content: Editable, lineStart: Int, newlineIndex: Int, body: String, prefix: String) {
        suppressEditorWatcher = true
        if (body.trim().isEmpty()) {
            val parent = quoteParentPrefix(prefix)
            if (parent.isEmpty()) {
                content.delete(lineStart, newlineIndex + 1)
                editor.setSelection(lineStart)
            } else {
                content.replace(lineStart, newlineIndex + 1, parent)
                editor.setSelection(lineStart + parent.length)
            }
        } else {
            val nextPrefix = normalizeQuotePrefix(prefix)
    ...
    private fun quoteParentPrefix(prefix: String): String {
        val indent = prefix.takeWhile { it == ' ' || it == '\t' }
        val count = Regex(">").findAll(prefix.substring(indent.length)).count()
        if (count <= 1) return ""
        return indent + (1 until count).joinToString("") { "> " }
    }

    private fun normalizeQuotePrefix(prefix: String): String {
        val indent = prefix.takeWhile { it == ' ' || it == '\t' }
        val count = Regex(">").findAll(prefix.substring(indent.length)).count()
        return indent + (1..count).joinToString("") { "> " }
    }

    private fun isInsideCodeFence(position: Int): Boolean {
        val before = editor.text.substring(0, position.coerceIn(0, editor.text.length))
        return Regex("(?m)^```.*$").findAll(before).count() % 2 == 1
    }

    private fun lineStart(position: Int): Int {
        var p = position.coerceIn(0, editor.text.length)
        while (p > 0 && editor.text[p - 1] != '\n') p--
        return p
    }

    private fun lineEnd(position: Int): Int {
        var p = position.coerceIn(0, editor.text.length)
        while (p < editor.text.length && editor.text[p] != '\n') p++
        return p
    }
    ...
    private fun notifyEditorMutation() {
        scheduleAutosave()
        scheduleLivePreview()
    }
    ...
    private fun scheduleLivePreview() {
        handler.removeCallbacks(livePreviewRunnable)
        if (currentMode == EditorMode.LIVE) handler.postDelayed(livePreviewRunnable, LIVE_PREVIEW_DELAY_MS)
    }

    private fun saveDocument() {
        val file = documentFile ?: return
        try {
            store.write(file, editor.text.toString())
            saveStatus.text = "已保存"
        } catch (_: Exception) {
