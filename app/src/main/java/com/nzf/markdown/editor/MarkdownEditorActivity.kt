...
class MarkdownEditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editor.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                if (!suppressEditorWatcher && !restoringState) {
                    scheduleAutosave()
                    scheduleLivePreview()
                }
            }
    ...
    private fun restorePendingState(state: Bundle?) {
        ...
        restoringState = true
        pendingRestoredMode = try { EditorMode.valueOf(state.getString(STATE_EDITOR_MODE) ?: EditorMode.EDIT.name) } catch (_: IllegalArgumentException) { EditorMode.EDIT }
    ...
    private fun shareDocument() {
        ...
        try {
            startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply {
                type = "text/plain"
                putExtra(Intent.EXTRA_TITLE, titleView.text.toString())
                putExtra(Intent.EXTRA_TEXT, markdown)
            }, "分享到"))
        } catch (_: Exception) { Toast.makeText(this, "没有可用于分享的应用", Toast.LENGTH_SHORT).show() }
    }
    ...
    private fun showEditor(requestFocus: Boolean = true) {
        currentMode = EditorMode.EDIT; editor.visibility = View.VISIBLE; preview.visibility = View.GONE; liveModeDivider.visibility = View.GONE; editorToolbar.visibility = View.VISIBLE; setModeSelection(EditorMode.EDIT); if (requestFocus) editor.requestFocus()
    }
    private fun showPreview(saveBeforeRender: Boolean = true) {
        currentMode = EditorMode.PREVIEW; handler.removeCallbacks(livePreviewRunnable); if (saveBeforeRender) saveDocument(); renderMarkdown(editor.text.toString()); editor.visibility = View.GONE; preview.visibility = View.VISIBLE; liveModeDivider.visibility = View.GONE; editorToolbar.visibility = View.GONE; setModeSelection(EditorMode.PREVIEW)
    }
    private fun showLivePreview(requestFocus: Boolean = true) {
        currentMode = EditorMode.LIVE; editor.visibility = View.VISIBLE; preview.visibility = View.VISIBLE; liveModeDivider.visibility = View.VISIBLE; editorToolbar.visibility = View.VISIBLE; setModeSelection(EditorMode.LIVE); renderMarkdown(editor.text.toString()); if (requestFocus) editor.requestFocus()
    }
    private fun setModeSelection(mode: EditorMode) {
        ...
        editButton.alpha = if (mode == EditorMode.EDIT) 1f else .55f; previewButton.alpha = if (mode == EditorMode.PREVIEW) 1f else .55f; liveButton.alpha = if (mode == EditorMode.LIVE) 1f else .55f
    }

    private fun toggleHeading() {
        val content = editor.text; val start = lineStart(editor.selectionStart); val end = lineEnd(editor.selectionEnd); val heading = Regex("^(\\s*)(#{1,6})\\s+")
        val lines = content.substring(start, end).split("\n"); val nonBlank = lines.filter { it.isNotBlank() }; if (nonBlank.isEmpty()) return
        val allH1 = nonBlank.all { heading.find(it)?.groupValues?.get(2) == "#" }
        val replacement = lines.joinToString("\n") { line -> if (line.isBlank()) line else if (allH1) line.replaceFirst(heading, "$1") else { val indent = line.takeWhile { it == ' ' || it == '\t' }; indent + "# " + line.substring(indent.length).replaceFirst(Regex("^#{1,6}\\s+"), "") } }
        replaceSelection(start, end, replacement, start, start + replacement.length)
    }

    private fun toggleSelectedLines(type: BlockType) {
        val start = lineStart(editor.selectionStart); val end = lineEnd(editor.selectionEnd); val lines = editor.text.substring(start, end).split("\n"); val nonBlank = lines.filter { it.isNotBlank() }; if (nonBlank.isEmpty()) return
        val allPrefixed = nonBlank.all { hasBlockPrefix(it, type) }; var orderedIndex = 1
        val replacement = lines.joinToString("\n") { line -> if (line.isBlank()) line else if (allPrefixed) removeBlockPrefix(line, type) else { val transformed = addBlockPrefix(line, type, orderedIndex); if (type == BlockType.ORDERED) orderedIndex++; transformed } }
        replaceSelection(start, end, replacement, start, start + replacement.length)
    }
    ...
    private fun toggleWrap(prefix: String, suffix: String) {
        val start = editor.selectionStart.coerceAtLeast(0); val end = editor.selectionEnd.coerceAtLeast(start); val content = editor.text; val selected = content.substring(start, end)
        if (selected.isNotEmpty() && selected.startsWith(prefix) && selected.endsWith(suffix) && selected.length >= prefix.length + suffix.length) { val unwrapped = selected.substring(prefix.length, selected.length - suffix.length); replaceSelection(start, end, unwrapped, start, start + unwrapped.length); return }
        if (isSelectionWrapped(content, start, end, prefix, suffix)) { suppressEditorWatcher = true; content.delete(end, end + suffix.length); content.delete(start - prefix.length, start); editor.setSelection(start - prefix.length, end - prefix.length); suppressEditorWatcher = false; notifyEditorMutation(); return }
        if (selected.isEmpty()) { suppressEditorWatcher = true; content.insert(start, prefix + suffix); editor.setSelection(start + prefix.length); suppressEditorWatcher = false; notifyEditorMutation() } else replaceSelection(start, end, prefix + selected + suffix, start + prefix.length, start + prefix.length + selected.length)
    }
    ...
    private fun insertLink() {
        val start = editor.selectionStart; val end = editor.selectionEnd; val selected = editor.text.substring(start, end); val url = "https://"
        if (selected.isEmpty()) { val label = "链接文本"; val value = "[$label]($url)"; replaceSelection(start, end, value, start + 1, start + 1 + label.length) } else { val value = "[$selected]($url)"; val urlStart = start + selected.length + 3; replaceSelection(start, end, value, urlStart, urlStart + url.length) }
    }
    ...
    private fun insertImage(uri: Uri) {
        val start = editor.selectionStart; val prefix = if (start > 0 && editor.text[start - 1] != '\n') "\n" else ""; val imageMarkdown = "![图片]($uri)"; val insertionEnd = start + prefix.length + imageMarkdown.length; val suffix = if (insertionEnd < editor.text.length && editor.text[start] != '\n') "\n" else ""; val value = prefix + imageMarkdown + suffix
        suppressEditorWatcher = true; editor.text.insert(start, value); editor.setSelection(start + value.length); suppressEditorWatcher = false; saveStatus.text = "图片已插入"; notifyEditorMutation()
    }
    ...
    private fun continueMarkdownBlock(content: Editable, newlineIndex: Int) {
        ...
        val previousLineStart = lineStart(newlineIndex); val previous = content.substring(previousLineStart, newlineIndex)
        ...
        val quote = Regex("^(\\s*(?:>\\s?)+)(.*)$").matchEntire(previous); if (quote != null) continueQuoteBlock(content, previousLineStart, newlineIndex, quote.groupValues[2], quote.groupValues[1])
    }
    ...
    private fun continueCodeFence(content: Editable, newlineIndex: Int) {
        ...
        val nextFenceStart = findNextCodeFence(newlineIndex + 1)
        if (nextFenceStart >= 0) {
            suppressEditorWatcher = true
            val afterInsertedNewline = newlineIndex + 1
            content.delete(afterInsertedNewline, nextFenceStart + 3)
            editor.setSelection(afterInsertedNewline)
            suppressEditorWatcher = false
            notifyEditorMutation()
        }
    }

    private fun findNextCodeFence(from: Int): Int {
        val source = editor.text.toString()
        val matcher = Regex("(?m)^```[ \\t]*(?:\\n|$)").find(source, from.coerceIn(0, source.length))
        return matcher?.range?.first ?: -1
    }
    ...
    private fun continueListBlock(content: Editable, lineStart: Int, newlineIndex: Int, body: String, indent: String, nextMarker: String) {
        suppressEditorWatcher = true
        if (body.trim().isEmpty()) { if (indent.isNotEmpty()) { content.replace(lineStart, newlineIndex + 1, indent); editor.setSelection(lineStart + indent.length) } else { content.delete(lineStart, newlineIndex + 1); editor.setSelection(lineStart) } } else { val prefix = indent + nextMarker; content.insert(newlineIndex + 1, prefix); editor.setSelection(newlineIndex + 1 + prefix.length) }
        suppressEditorWatcher = false; notifyEditorMutation()
    }
    private fun continueQuoteBlock(content: Editable, lineStart: Int, newlineIndex: Int, body: String, prefix: String) {
        suppressEditorWatcher = true
        if (body.trim().isEmpty()) { val parent = quoteParentPrefix(prefix); if (parent.isEmpty()) { content.delete(lineStart, newlineIndex + 1); editor.setSelection(lineStart) } else { content.replace(lineStart, newlineIndex + 1, parent); editor.setSelection(lineStart + parent.length) } } else { val nextPrefix = normalizeQuotePrefix(prefix); content.insert(newlineIndex + 1, nextPrefix); editor.setSelection(newlineIndex + 1 + nextPrefix.length) }
        suppressEditorWatcher = false; notifyEditorMutation()
    }
