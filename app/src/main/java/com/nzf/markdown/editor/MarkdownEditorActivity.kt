...
class MarkdownEditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        preview.webViewClient = object : WebViewClient() { override fun onPageFinished(view: WebView?, url: String?) { previewReady = true; applyRestoredMode() } }
        ...
        editor.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) { if (suppressEditorWatcher || s == null || insertedNewlineIndex < 0) return; val index = insertedNewlineIndex; insertedNewlineIndex = -1; continueMarkdownBlock(s, index) }
        })
        if (savedInstanceState == null) showEditor() else applyRestoredMode()
    }

    private fun restorePendingState(state: Bundle?) { if (state == null) return; restoringState = true; pendingRestoredMode = try { EditorMode.valueOf(state.getString(STATE_EDITOR_MODE) ?: EditorMode.EDIT.name) } catch (_: IllegalArgumentException) { EditorMode.EDIT }; pendingSelectionStart = state.getInt(STATE_SELECTION_START, editor.text.length).coerceIn(0, editor.text.length); pendingSelectionEnd = state.getInt(STATE_SELECTION_END, pendingSelectionStart).coerceIn(0, editor.text.length); pendingEditorScrollX = state.getInt(STATE_EDITOR_SCROLL_X, 0); pendingEditorScrollY = state.getInt(STATE_EDITOR_SCROLL_Y, 0); pendingPreviewScrollY = if (state.containsKey(STATE_PREVIEW_SCROLL_Y)) state.getInt(STATE_PREVIEW_SCROLL_Y) else null; editor.post { editor.setSelection(pendingSelectionStart, pendingSelectionEnd); editor.scrollTo(pendingEditorScrollX, pendingEditorScrollY); restoringState = false } }
    private fun applyRestoredMode() { val mode = pendingRestoredMode ?: return; pendingRestoredMode = null; when (mode) { EditorMode.EDIT -> showEditor(false); EditorMode.PREVIEW -> showPreview(false); EditorMode.LIVE -> showLivePreview(false) }; restoreEditorViewport() }
    private fun restoreEditorViewport() { if (pendingSelectionStart < 0) return; editor.post { editor.setSelection(pendingSelectionStart.coerceIn(0, editor.text.length), pendingSelectionEnd.coerceIn(0, editor.text.length)); editor.scrollTo(pendingEditorScrollX, pendingEditorScrollY) } }
    ...
    private fun shareDocument() { val markdown = editor.text.toString(); if (markdown.trim().isEmpty()) { Toast.makeText(this, "文档内容为空，暂时无法分享", Toast.LENGTH_SHORT).show(); return }; saveDocument(); try { startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = "text/plain"; putExtra(Intent.EXTRA_TITLE, titleView.text.toString()); putExtra(Intent.EXTRA_TEXT, markdown) }, "分享到")) } catch (_: Exception) { Toast.makeText(this, "没有可用于分享的应用", Toast.LENGTH_SHORT).show() } }
    ...
    private fun toggleHeading() { val content = editor.text; val start = lineStart(editor.selectionStart); val end = lineEnd(editor.selectionEnd); val heading = Regex("^(\\s*)(#{1,6})\\s+"); val lines = content.substring(start, end).split("\n"); val nonBlank = lines.filter { it.isNotBlank() }; if (nonBlank.isEmpty()) return; val allH1 = nonBlank.all { heading.find(it)?.groupValues?.get(2) == "#" }; val replacement = lines.joinToString("\n") { line -> if (line.isBlank()) line else if (allH1) line.replaceFirst(heading, "$1") else { val indent = line.takeWhile { it == ' ' || it == '\t' }; indent + "# " + line.substring(indent.length).replaceFirst(Regex("^#{1,6}\\s+"), "") } }; replaceSelection(start, end, replacement, start, start + replacement.length) }
    private fun toggleSelectedLines(type: BlockType) { val start = lineStart(editor.selectionStart); val end = lineEnd(editor.selectionEnd); val lines = editor.text.substring(start, end).split("\n"); val nonBlank = lines.filter { it.isNotBlank() }; if (nonBlank.isEmpty()) return; val allPrefixed = nonBlank.all { hasBlockPrefix(it, type) }; var orderedIndex = 1; val replacement = lines.joinToString("\n") { line -> if (line.isBlank()) line else if (allPrefixed) removeBlockPrefix(line, type) else { val transformed = addBlockPrefix(line, type, orderedIndex); if (type == BlockType.ORDERED) orderedIndex++; transformed } }; replaceSelection(start, end, replacement, start, start + replacement.length) }
    private fun hasBlockPrefix(line: String, type: BlockType): Boolean = when (type) { BlockType.QUOTE -> Regex("^\\s*(?:>\\s?)+").containsMatchIn(line); BlockType.UNORDERED -> Regex("^(\\s*)[-+*]\\s+").containsMatchIn(line); BlockType.ORDERED -> Regex("^(\\s*)\\d+[.)]\\s+").containsMatchIn(line); else -> false }
    private fun removeBlockPrefix(line: String, type: BlockType): String = when (type) { BlockType.QUOTE -> line.replaceFirst(Regex("^(\\s*)>\\s?"), "$1"); BlockType.UNORDERED -> line.replaceFirst(Regex("^(\\s*)[-+*]\\s+"), "$1"); BlockType.ORDERED -> line.replaceFirst(Regex("^(\\s*)\\d+[.)]\\s+"), "$1"); else -> line }
    private fun addBlockPrefix(line: String, type: BlockType, orderedIndex: Int): String { val indent = line.takeWhile { it == ' ' || it == '\t' }; val body = line.substring(indent.length); return when (type) { BlockType.QUOTE -> indent + "> " + body; BlockType.UNORDERED -> indent + "- " + body; BlockType.ORDERED -> indent + "$orderedIndex. " + body; else -> line } }
    ...
    private fun toggleCode() { val start = editor.selectionStart; val end = editor.selectionEnd; val selected = editor.text.substring(start, end); if (selected.contains("\n")) { if (selected.startsWith("```\n") && selected.endsWith("\n```")) { val unwrapped = selected.substring(4, selected.length - 4); replaceSelection(start, end, unwrapped, start, start + unwrapped.length) } else replaceSelection(start, end, "```\n$selected\n```", start + 4, start + 4 + selected.length); return }; if (selected.isNotEmpty()) { toggleWrap("`", "`"); return }; if (lineStart(start) == lineEnd(start)) insertCodeBlock(start) else toggleWrap("`", "`") }
    private fun insertCodeBlock(start: Int) { replaceSelection(start, start, "```\n\n```", start + 4, start + 4) }
    private fun insertLink() { val start = editor.selectionStart; val end = editor.selectionEnd; val selected = editor.text.substring(start, end); val url = "https://"; if (selected.isEmpty()) { val label = "链接文本"; val value = "[$label]($url)"; replaceSelection(start, end, value, start + 1, start + 1 + label.length) } else { val value = "[$selected]($url)"; val urlStart = start + selected.length + 3; replaceSelection(start, end, value, urlStart, urlStart + url.length) } }
    ...
    private fun insertImage(uri: Uri) { val start = editor.selectionStart; val prefix = if (start > 0 && editor.text[start - 1] != '\n') "\n" else ""; val imageMarkdown = "![图片]($uri)"; val insertionEnd = start + prefix.length + imageMarkdown.length; val suffix = if (insertionEnd < editor.text.length && editor.text[start] != '\n') "\n" else ""; val value = prefix + imageMarkdown + suffix; suppressEditorWatcher = true; editor.text.insert(start, value); editor.setSelection(start + value.length); suppressEditorWatcher = false; saveStatus.text = "图片已插入"; notifyEditorMutation() }
    private fun continueMarkdownBlock(content: Editable, newlineIndex: Int) { if (isInsideCodeFence(newlineIndex)) { continueCodeFence(content, newlineIndex); return }; val previousLineStart = lineStart(newlineIndex); val previous = content.substring(previousLineStart, newlineIndex); val ordered = Regex("^(\\s*)(\\d+)([.)])\\s*(.*)$").matchEntire(previous); if (ordered != null) { continueListBlock(content, previousLineStart, newlineIndex, ordered.groupValues[4], ordered.groupValues[1], "${ordered.groupValues[2].toIntOrNull()?.plus(1) ?: 2}${ordered.groupValues[3]} "); return }; val unordered = Regex("^(\\s*)([-+*])\\s*(.*)$").matchEntire(previous); if (unordered != null) { continueListBlock(content, previousLineStart, newlineIndex, unordered.groupValues[3], unordered.groupValues[1], unordered.groupValues[2] + " "); return }; val quote = Regex("^(\\s*(?:>\\s?)+)(.*)$").matchEntire(previous); if (quote != null) continueQuoteBlock(content, previousLineStart, newlineIndex, quote.groupValues[2], quote.groupValues[1]) }
    private fun continueCodeFence(content: Editable, newlineIndex: Int) { val previousLineStart = lineStart(newlineIndex); val previous = content.substring(previousLineStart, newlineIndex); if (previous.isNotEmpty()) return; val nextFenceStart = findNextCodeFence(newlineIndex + 1); if (nextFenceStart >= 0) { suppressEditorWatcher = true; val afterInsertedNewline = newlineIndex + 1; content.delete(afterInsertedNewline, nextFenceStart + 3); editor.setSelection(afterInsertedNewline); suppressEditorWatcher = false; notifyEditorMutation() } }
    private fun findNextCodeFence(from: Int): Int { val source = editor.text.toString(); val matcher = Regex("(?m)^```[ \\t]*(?:\\n|$)").find(source, from.coerceIn(0, source.length)); return matcher?.range?.first ?: -1 }
    ...
    private fun saveDocument() { val file = documentFile ?: return; try { store.write(file, editor.text.toString()); saveStatus.text = "已保存" } catch (_: Exception) { saveStatus.text = "保存失败" } }
    private fun renderMarkdown(markdown: String) { if (!previewReady) return; preview.evaluateJavascript("renderMarkdown(${JSONObject.quote(markdown)});", null); restorePreviewScrollWhenReady() }
    private fun restorePreviewScrollWhenReady() { val target = pendingPreviewScrollY ?: return; preview.postDelayed({ if (!previewReady) return@postDelayed; preview.evaluateJavascript("(function(){window.scrollTo(0, ${target.coerceAtLeast(0)}); return window.scrollY || window.pageYOffset || 0;})()") { pendingPreviewScrollY = null } }, 120) }
    private fun handleBackNavigation() { if (isKeyboardVisible()) { hideKeyboard(); return }; saveDocument(); finish() }
    private fun isKeyboardVisible(): Boolean { val visibleFrame = Rect(); window.decorView.getWindowVisibleDisplayFrame(visibleFrame); val rootHeight = window.decorView.rootView.height; return rootHeight > 0 && rootHeight - visibleFrame.bottom > rootHeight / 6 }
    private fun hideKeyboard() { val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager; imm.hideSoftInputFromWindow(editor.windowToken, 0); editor.clearFocus() }
    override fun onBackPressed() { if (isKeyboardVisible()) { hideKeyboard(); return }; saveDocument(); super.onBackPressed() }
    override fun onSaveInstanceState(outState: Bundle) { outState.putString(STATE_EDITOR_MODE, currentMode.name); outState.putInt(STATE_SELECTION_START, editor.selectionStart.coerceAtLeast(0)); outState.putInt(STATE_SELECTION_END, editor.selectionEnd.coerceAtLeast(0)); outState.putInt(STATE_EDITOR_SCROLL_X, editor.scrollX); outState.putInt(STATE_EDITOR_SCROLL_Y, editor.scrollY); if (previewReady) preview.evaluateJavascript("(function(){return String(window.scrollY || window.pageYOffset || 0);})()") { value -> value.trim().trim('"').toIntOrNull()?.let { pendingPreviewScrollY = it } }; pendingPreviewScrollY?.let { outState.putInt(STATE_PREVIEW_SCROLL_Y, it) }; super.onSaveInstanceState(outState) }
